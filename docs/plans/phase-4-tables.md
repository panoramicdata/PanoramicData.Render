...
- [ ] 4.1.1 — Parse table structure from OpenXML: `w:tbl`, `w:tr`, `w:tc`, grid columns (`w:tblGrid`)
- [ ] 4.1.2 — Parse table properties: width (fixed, percentage, auto), alignment (left, center, right), indentation
- [ ] 4.1.3 — Parse row properties: height (exact, at-least, auto), header row (repeat on each page), `cantSplit`
- [ ] 4.1.4 — Parse cell properties: width, vertical alignment (top, center, bottom), text direction, margins/padding
- [ ] 4.1.5 — Parse cell merging: horizontal merge (`w:gridSpan`), vertical merge (`w:vMerge` start/continue)
- [ ] 4.1.6 — Unit tests: verify table model correctly represents all test documents

### 4.2 Fixed Layout Tables

- [ ] 4.2.1 — Implement fixed-width table layout: column widths are specified explicitly in the grid
- [ ] 4.2.2 — Compute cell positions: $(x, y)$ for each cell based on grid column offsets and accumulated row heights
- [ ] 4.2.3 — Lay out cell content using the text layout engine (cells contain paragraphs, possibly nested tables)
- [ ] 4.2.4 — Handle cell margins (top, bottom, left, right padding)
- [ ] 4.2.5 — Handle vertical alignment within cells (top, center, bottom)
- [ ] 4.2.6 — Unit tests: verify cell positions and content layout

### 4.3 Auto-Fit Tables

- [ ] 4.3.1 — Implement the auto-fit algorithm: measure preferred/minimum/maximum widths for each column
